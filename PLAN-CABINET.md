...
5. **Rien ne prouve qui a envoyé un paquet, et l'application certifie quand même « vérifiées, intactes »** (C1). Le paquet est chiffré **pour** le cabinet avec sa clé publique — que n'importe qui peut avoir. L'empreinte prouve que le fichier n'a pas bougé depuis sa fabrication, pas **qui** l'a fabriqué. → **corrigé en 9.2.0** : le client signe les octets exacts de `manifeste.json` avec une paire Ed25519 créée à son premier envoi, le cabinet épingle la clé au premier paquet signé, et `cabcore.verdictOrigine` refuse ensuite tout paquet non signé ou signé d'une autre clé — la tolérance s'éteint d'elle-même, client par client, sans date butoir.
...
- **La démonstration complète** — ~~**B10** Écritures, la fonction qui fait gagner des heures, est **vide pendant toute la démonstration**.~~ *Fait : depuis la 9.2.2 l'exemple livre de vrais paquets, et depuis la 9.4.3 la page les regroupe — vérifié à l'écran le 23/09/2026 (10.12.0).*
