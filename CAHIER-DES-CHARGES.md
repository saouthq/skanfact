...
| `ERR-CAB-062` | « Cet exercice ne porte aucun solde à reporter. » / « Les à-nouveaux ne s'équilibrent pas… » / « Les à-nouveaux de <année> sont déjà validés. Contre-passe-les si le report a changé. » | ouverture de l'exercice suivant (9.8.0) ; depuis la 10.14.0 l'écran ne le déclenche plus — le bouton lit ce que ferait le geste (`etatExerciceSuivant`) : « Voir les à-nouveaux » quand tout est reporté, éteint avec le motif quand le report est impossible | contre-passer, ou corriger la balance | Livré |
| `ERR-CAB-063` | « Cet exercice ne porte aucun à-nouveau : il n'y aurait rien à envoyer au client. » | `.skanclose` (9.8.0) | — | Livré |
| `ERR-CAB-064` | « L'exercice <année> est clos : plus rien n'y bouge. Rouvre-le (onglet Exercice, motif exigé) si tu dois vraiment y toucher — la réouverture est la seule trace qui expliquera pourquoi un chiffre a changé après coup. » | toute écriture dans un exercice clos, à la porte unique `ecrireLeLivre` (10.14.0) : écritures, biens, bulletins, relevés, déclarations, inventaires, plan | rouvrir l'exercice, motif exigé | Livré |
...
| `ERR-ENT-086` | « Le serveur de SkanFact ne répond pas… » / le refus du serveur tel quel (« Indique une adresse e-mail : c'est par là que la clé arrive. ») | « Acheter » depuis Paramètres → Licence (10.14.0) | compléter la fiche société, ou demander la licence par mail | Livré |
...
| `ERR-CAB-082` | (motif de `analyserImportEcritures` : pas de colonne « Compte », « Date » ou de montant ; classeur au lieu d'un CSV) | import d'écritures depuis un tableur (10.14.1) | enregistrer le tableur en CSV, garder la ligne d'entête de l'export | Livré |
