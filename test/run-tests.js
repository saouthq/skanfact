...
t('cabinet : la clé privée ne traverse jamais le pont vers l\'interface', () => {
  ...
  // 9.4.10 — et il lit du CODE. Troisième fois que le motif revient (6.8.0, 7.25.0) : un commentaire
  // qui CITE « ipcMain.handle( » pour expliquer la règle fabriquait un faux handler, sans nom, et
  // faisait tomber ce test-ci sur du code parfaitement juste. Les commentaires se retirent d'abord,
  // et on vérifie que le nettoyage n'a pas mangé le code.
  const net = src.replace(/\/\*[\s\S]*?\*\//g, '').split('\n').filter(l => !/^\s*\/\//.test(l)).join('\n');
  assert.ok(net.includes('function safeState()') && net.includes('ipcMain.handle('),
    'le retrait des commentaires a mangé le code du cabinet');
  const blocs = net.split(/ipcMain\.handle\(/).slice(1);
...
(async () => {
  ...
  // ---------- les suites découpées par domaine (9.4.10) ----------
  // Le fichier faisait 13 900 lignes : personne ne tient ça en tête, et une tranche de source
  // se casse à chaque déménagement. On découpe par occasion, un domaine à la fois. Chaque suite
  ...
  // sont là, ils affichent « ok » nulle part, et on croit être couvert. Le dossier fait foi.
  t('9.4.10 : chaque suite de test/suites est chargée par le lanceur', () => {
    const fichiers = fs.readdirSync(path.join(__dirname, 'suites')).filter(f => f.endsWith('.js')).sort();
    assert.ok(fichiers.length >= 1, 'plus aucune suite découpée : le contrôle ne garde plus rien');
    const lanceur = lireSource('test', 'run-tests.js');
    const oubliees = fichiers.filter(f => !lanceur.includes(`require('./suites/${f}')`));
    assert.deepStrictEqual(oubliees, [], 'suite(s) écrite(s) et jamais chargée(s)');
  });

  // ---------- 9.4.10 : les feuilles de style en propriétés LOGIQUES ----------
  // `margin-left` dit « le bord gauche de l'écran » ; `margin-inline-start` dit « le bord d'où
  // commence la lecture ». En français les deux donnent la même image ; en arabe, la seconde suit
  // le texte et la première ne suit rien. Ça ne promet pas l'arabe — ça garde la porte ouverte
  // pour deux heures de travail aujourd'hui au lieu d'un chantier dans deux ans (QUESTIONS.md § 3).
  t('9.4.10 : aucune déclaration CSS physique dans les deux feuilles', () => {
    // Les rares lignes qui gardent une propriété physique, chacune avec sa raison. Une exception
    // ANONYME est un trou : on nomme le sélecteur, sinon la règle s'effrite au premier ajout.
    const GARDEES = [
      // `left: 50%` + `translateX(-50%)` centre horizontalement. `left` est ici SYMÉTRIQUE : il vaut
      // dans les deux sens de lecture, alors qu'`inset-inline-start: 50%` collerait le bandeau à
      // droite en arabe. Le physique est le juste, ici.
      { fichier: 'style.css', selecteur: '#toast' },
    ];
    const PHYS = /(?:^|[;{\s])(?:(?:margin|padding|border)-(?:left|right)(?:-[a-z]+)?|left|right)\s*:|text-align\s*:\s*(?:left|right)\b|float\s*:\s*(?:left|right)\b/;
    for (const [nom, chemin] of [['style.css', ['src', 'renderer', 'style.css']],
                                 ['cabinet.css', ['src', 'cabinet', 'renderer', 'cabinet.css']]]) {
      const brut = lireSource(...chemin);
      const net = brut.replace(/\/\*[\s\S]*?\*\//g, '');
      // Le nettoyage n'a pas mangé le code (6.8.0) : la feuille porte toujours ses règles.
      assert.ok(net.includes('.btn {') || net.includes('.panel {') || net.includes('.warn-box {'),
        nom + ' : le retrait des commentaires a mangé la feuille');
      const fautes = [];
      net.split('\n').forEach((l, i) => {
        if (!PHYS.test(l)) return;
        if (GARDEES.some(g => g.fichier === nom && l.includes(g.selecteur))) return;
        fautes.push((i + 1) + ' : ' + l.trim().slice(0, 90));
      });
      assert.deepStrictEqual(fautes, [], nom + ' : déclaration(s) physique(s), à écrire en logique');
  ...
  // `money` écrit un négatif « − 1 234,500 DT » avec le signe moins typographique (U+2212). Cinq
  // parcours le nettoyaient avec `[^\d,.-]`, qui ne garde que le trait d'union ASCII : le signe
  // disparaissait, « − 4 000 » devenait quatre mille, et un tri décroissant paraissait non trié —
  // certains jours seulement, puisque le jeu d'exemple est relatif à aujourd'hui. Un parcours qui
  // tombe un jour sur deux sur du code juste finit par ne plus être relancé.
  t('9.4.10 : aucun parcours ne relit un montant en perdant son signe', () => {
    ...
    // l'est. Quatrième fois que ce piège se referme dans ce dépôt (6.8.0, 7.25.0, 9.4.10 plus haut).
    const fautifs = fs.readdirSync(dossier).filter(f => f.endsWith('.js')).filter(f => {
      const net = fs.readFileSync(path.join(dossier, f), 'utf8')
        .replace(/\/\*[\s\S]*?\*\//g, '').split('\n').filter(l => !/^\s*\/\//.test(l)).join('\n');
      return /\[\^\\d,\.-\]/.test(net);
    });
    assert.deepStrictEqual(fautifs, [], 'parcours qui relisent un montant sans son signe — passer par montant()');
  });

  // ---------- 9.4.10 : chaque refus porte son code, et le code existe dans le cahier ----------
  // « Il m'a dit que le paquet est illisible » ne désigne aucune ligne de code ; « ERR-CAB-010 » si.
  // Deux moitiés, et il faut les deux : le code posé sur le refus, et la table du cahier qui le
  // nomme. Deux tables qui vivent séparément divergent, toujours (6.8.0, 7.23.0).
  t('9.4.10 : tout refus des deux main.js porte un code, présent dans le cahier', () => {
    // Les refus INTERNES : attrapés sur place et reformulés en une phrase écrite pour l'utilisateur.
    // Ils ne remontent jamais tels quels, donc un code n'y servirait à personne. Nommés, parce
    // qu'une exception anonyme est un trou.
    const INTERNES = [
      "throw new Error('HTTP ' + r.status)",              // requête au relais : `postPlateforme` l'attrape
      "throw new Error('réponse illisible')",             // idem
      "throw new Error('document non rendu')"             // fabrication du paquet : le manifeste dit `absents`
    ];
    const codesUtilises = new Set();
    for (const [nom, chemin] of [['src/main.js', ['src', 'main.js']],
                                 ['src/cabinet/main.js', ['src', 'cabinet', 'main.js']]]) {
      const src = lireSource(...chemin);
      const net = src.replace(/\/\*[\s\S]*?\*\//g, '').split('\n').filter(l => !/^\s*\/\//.test(l)).join('\n');
      assert.ok(net.includes('const erreur = (code, message)'), nom + ' : la fabrique d\'erreurs a disparu');
      // Le code voyage DANS le message : une propriété posée sur une Error ne traverse pas le pont.
      assert.ok(/new Error\(`\$\{message\} \[\$\{code\}\]`\)/.test(net),
        nom + ' : le code ne voyage plus dans le message, donc l\'écran ne le verra jamais');
      // Et ce qu'aucune phrase n'attendait laisse une trace : l'enveloppe posée UNE fois sur
      // `ipcMain.handle`. Elle ne journalise que ce qui n'est pas un refus écrit — sinon le journal
      // se remplit de mots de passe mal tapés, et le parcours `e2e:entreprise`, qui exige un
      // `main.log` VIDE après une exécution propre, tombe sur du code juste.
      assert.ok(/ipcMain\.handle = \(canal, fn\)/.test(net), nom + ' : l\'enveloppe des handlers a disparu');
      assert.ok(/if \(!\(e && e\.refus\)\) logToFile\('panne '/.test(net),
        nom + ' : une exception imprévue ne laisse plus de trace dans le journal');
      ...
      assert.deepStrictEqual(nus, [], 'refus sans code — passer par erreur(\'ERR-…\', …)');
